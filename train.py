...
Usage (standalone):
    uv run train.py      # 3-lambda sweep, 25 epochs (10 warmup + 15 prune)
    python train.py

Usage (modular):
    from train import train_one_epoch, evaluate, run_training, calculate_sparsity
"""
...
# ──────────────────────────────────────────────────────────────
# Shared model import (inline fallback for standalone execution)
# ──────────────────────────────────────────────────────────────
...
try:
    ...
    class PrunableLinear(nn.Module):
        """Custom linear layer with learnable sigmoid gates (inline fallback)."""
    ...
    class SelfPruningNet(nn.Module):
        """Feed-forward MLP for CIFAR-10 using PrunableLinear layers (inline fallback)."""
        ...
        def calculate_sparsity_penalty(self):
            """Raw L1 sum of gate values (not normalised — see model.py for rationale)."""
            device = next(self.parameters()).device
            total = torch.tensor(0.0, device=device)
            for m in self.modules():
                if isinstance(m, PrunableLinear):
                    total += torch.sigmoid(m.gate_scores).sum()
            return total   # raw sum; use small λ (0.1–5.0 with warmup)
...
def train_one_epoch(model, loader, optimizer, criterion,
    ...
    If lambda_sparsity == 0 (warmup phase), only CE loss is used.
    """
    model.train()
...
def evaluate(model, loader, criterion, device) -> tuple:
    """Evaluate on held-out set. Returns (avg_loss, accuracy_percent)."""
    model.eval()
    running_loss, correct, total = 0.0, 0, 0
...
def calculate_sparsity(model, threshold: float = 0.01) -> float:
    """
    Return fraction of gate values below *threshold* (effectively pruned).

    Args:
        model     : Model with PrunableLinear layers.
        threshold : Gates below this are counted as pruned (default 0.01).
...
# ──────────────────────────────────────────────────────────────
# 4. CONVENIENCE WRAPPER (used by main.py / full_run.py)
# ──────────────────────────────────────────────────────────────
...
def run_training(
    ...
    """
    Two-phase training loop with CosineAnnealingLR scheduling.

    Phase 1 — Warmup (epochs 1..warmup_epochs, λ=0):
        Train with CrossEntropy only. Network converges; important weights
        grow large; unimportant weights stay small.

    Phase 2 — Prune (epochs warmup+1..num_epochs, full λ):
        Add sparsity penalty. Unimportant gates (backed by small weights,
        near-zero CE gradient) get pushed to 0 by Adam's consistent
        −lr updates. Important gates resist via CE gradient.

    Returns a history dict with keys:
        train_loss, train_acc, test_loss, test_acc, sparsity, phase
    """
    ...
    optimizer = optim.Adam([
        ...
        {"params": gate_params,   "lr": 0.05}  # 50x higher LR so gates can reach 0
    ])
    ...
    for epoch in range(1, num_epochs + 1):
        lam         = 0.0 if epoch <= warmup_epochs else lambda_sparsity
        phase_label = "warmup" if epoch <= warmup_epochs else "prune"

        tr_loss, tr_acc = train_one_epoch(
            model, train_loader, optimizer, criterion, device, lam)
        ...
        if epoch % 5 == 0 or epoch == 1 or epoch == warmup_epochs + 1:
            print(
                f"  [{phase_label:6s}] Epoch {epoch:02d}/{num_epochs} | "
                f"Loss: {tr_loss:.4f} | "
...
def plot_gate_distribution(model, lambda_val: float, save_path: str):
    ...
    After successful two-phase training, this should show a BIMODAL
    distribution — a spike near 0 (pruned) and a cluster near 0.5-1.0
    (active), with a clear gap between them.
    """
    all_gates = []
    with torch.no_grad():
        for module in model.modules():
...
def plot_training_history(history: dict, lambda_val: float, save_path: str):
    """
    Three-panel plot: Loss | Accuracy | Sparsity over epochs.
    A vertical dashed line marks the warmup→prune transition.
    """
    epochs = range(1, len(history["train_loss"]) + 1)
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))

    # Detect warmup boundary
    warmup_end = None
    if "phase" in history:
...
# ──────────────────────────────────────────────────────────────
# 6. STANDALONE MAIN — TWO-PHASE 3-λ SWEEP
# ──────────────────────────────────────────────────────────────

def main():
    """
    Two-phase 3-λ sweep (25 epochs = 10 warmup + 15 prune):

    Lambda values: 1e-6 | 1e-5 | 1e-4
      - λ=1e-6 → mild pruning    → highest accuracy
      - λ=1e-5 → balanced        → balanced trade-off
      - λ=1e-4 → aggressive      → most sparsity (~99%)
    ...
    for lam in LAMBDA_VALUES:
        print(f"\n{'═' * 55}")
        print(f"  λ = {lam}  "
              f"(warmup {WARMUP_EPOCHS} epochs → prune {EPOCHS - WARMUP_EPOCHS} epochs)")
        print(f"{'═' * 55}")
